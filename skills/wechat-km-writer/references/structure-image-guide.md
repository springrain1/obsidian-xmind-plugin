...
每篇文章推荐生成一张内容结构图，放在封面图之后。直接使用 Antigravity 内置的 `generate_image` 工具，传入描述性的 Prompt 即可。

> ⚠️ **容错规则**：`generate_image` 依赖外部模型，可能因容量不足（503）等原因失败。
> - **最多重试 1 次**，仍失败则跳过，在 Markdown 中使用占位符 `<!-- 内容结构图待补充 -->`
> - 不要因为图片生成失败而阻塞文章的输出
>
> ⚠️ **前置依赖**：本步骤需要从**已完成的文章**中提取要点，因此必须在文章写作完成后才能执行。

## 图片部署

`generate_image` 工具生成的图片保存在系统目录中，需要复制到项目目录并重命名为 `structure.png`。
...
**API 返回 503 / 容量不足**：
- 最多重试 1 次，仍失败则跳过，使用占位符 `<!-- 内容结构图待补充 -->`
