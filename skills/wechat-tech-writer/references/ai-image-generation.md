...
## 🚀 快速开始：使用 generate_image 工具

直接使用 Antigravity 内置的 `generate_image` 工具生成图片，**无需配置外部 API 密钥或调用脚本**。

**基本用法**：

只需提供描述性的 Prompt，工具会自动生成图片并保存为 artifact。
...
# 1. 确定需要的图片类型和提示词
images_needed = [
    {"type": "cover", "prompt": "封面图提示词"},
    {"type": "comparison", "prompt": "对比图提示词（如有需要）"},
]

# 2. 使用 generate_image 工具逐个生成
# generate_image(prompt="封面图提示词", image_name="cover")
# generate_image(prompt="对比图提示词", image_name="comparison")
