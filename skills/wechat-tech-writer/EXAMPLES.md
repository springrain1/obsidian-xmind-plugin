...
**调用生成**：使用 Antigravity 内置的 `generate_image` 工具，传入上述提示词即可。
...
**生成**：使用 Antigravity 内置的 `generate_image` 工具，传入上述提示词即可。
...
**调用生成**：使用 Antigravity 内置的 `generate_image` 工具，传入上述提示词即可。即使用户要求使用 DALL-E，也统一由 `generate_image` 工具处理。
...
3. **图片生成说明**
   - 统一使用 Antigravity 内置的 `generate_image` 工具
   - 只需提供描述性的 Prompt 即可
   - 无需关心底层 API 选择
