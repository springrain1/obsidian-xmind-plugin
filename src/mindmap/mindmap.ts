...
export default class MindMap {
    ...
    refresh(preserveViewState: boolean = false) {
        this.layout(preserveViewState);
    }
    ...
    updateNodeData() {
        // 重新构建数据结构
        const buildNodeData = (node: INode): INodeData => {
            const data: INodeData = {
                id: node.data.id,
                text: node.data.text,
                pid: node.data.pid,
                isRoot: node.data.isRoot,
                expanded: node.isExpand
            };

            if (node.children && node.children.length > 0) {
                data.children = node.children.map(child => buildNodeData(child));
            }
        ...
        // 更新根节点数据
        if (this.root) {
            this.data = buildNodeData(this.root);
        }
    }
    ...
    center() {
        ...
        const translateX = (contentWidth - containerWidth) / 2
        const translateY = (contentHeight - containerHeight) / 2
        this.containerEL.scrollLeft = translateX
        this.containerEL.scrollTop = translateY
    }

    centerOnNode(node: INode, smooth: boolean = false) {
        if (!node) return;

        // 获取节点和容器的位置和尺寸信息
        const nodeBox = node.getBox();
        const containerWidth = this.containerEL.clientWidth;
        const containerHeight = this.containerEL.clientHeight;
        ...
        // 计算滚动位置，将节点放置在视口中央
        const scrollLeft = nodeBox.x + nodeBox.width / 2 - containerWidth / 2;
        const scrollTop = nodeBox.y + nodeBox.height / 2 - containerHeight / 2;
