...
export default class Layout {
    ...
    layoutRight(arr?:Node[],parent?:Node){
       ...
       // 对于包含图片的节点增加额外间距
       const extraPadding = this._checkIfNodesContainImages(rights) ? 20 : 0;
       nodeDis += extraPadding;
       ...
       rights.forEach((child) => {
           // 检查节点是否包含图片，如果是则增加垂直间距
           const childHasImage = this._nodeContainsImage(child);
           child.setPosition(next[0], next[1]);
           var childBox = child.getBox();
           // 为包含图片的节点增加额外的垂直空间
           next[1] += parseInt(childBox.height + nodeDis + (childHasImage ? 15 : 0) +'');
       });
    ...
    layoutLeft(arr?:Node[],parent?:Node){
        ...
        // 对于包含图片的节点增加额外间距
        const extraPadding = this._checkIfNodesContainImages(lefts) ? 20 : 0;
        nodeDis += extraPadding;
       ...
       // var next = [parseInt(pos.x - dis+''), parseInt(pos.y - disHeight / 2 +'')];
        lefts.forEach((child) => {
            ...
            // 为包含图片的节点增加额外的垂直空间
            next[1] += childBox.height + nodeDis + (childHasImage ? 15 : 0);
        });
...
child._barDom.setCssProps({ '--mm-node-stroke': _stroke });
    ...
    // 新增：检查节点是否包含图片元素
    _nodeContainsImage(node: Node): boolean {
        try {
            const contentEl = node.containEl.querySelector('.mm-node-text');
            if (!contentEl) return false;
            
            // 检查是否包含img标签
            return !!contentEl.querySelector('img');
        } catch (e) {
            console.error('检查节点图片时出错:', e);
            return false;
        }
    }

    // 新增：检查节点数组中是否有任何节点包含图片
    _checkIfNodesContainImages(nodes: Node[]): boolean {
        if (!nodes || !nodes.length) return false;
        
        for (const node of nodes) {
            if (this._nodeContainsImage(node)) {
                return true;
            }
        }
        return false;
