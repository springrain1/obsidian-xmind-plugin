...
const DEFAULT_SILICONFLOW_MODELS: AIModel[] = [
...
export class SiliconFlowSettings extends BaseAIServiceSettings {
    ...
    display(containerEl: HTMLElement): void {
        const siliconflowSettingsContainer = containerEl.createEl('div', {
            cls: 'ai-service-settings'
        });

        // 添加标题
        new Setting(siliconflowSettingsContainer)
            .setName('SiliconFlow 服务')
            .setHeading();
        ...
        new Setting(siliconflowSettingsContainer)
            .setName('API 密钥')
            .setDesc('请输入您的 SiliconFlow API 密钥')
            .addText(text => text
                .setPlaceholder('sk-...')
        ...
        // 模型选择设置
        const modelSetting = new Setting(siliconflowSettingsContainer)
            .setName('模型')
            ...
            .addText(text => text
                ...
                .onChange(async (value) => {
                    ...
                    if (!/^[a-zA-Z0-9-_.\/]+$/.test(trimmedValue)) {
                        new Notice('模型 ID 只能包含字母、数字、下划线、点、连字符和斜杠。');
        ...
        new Setting(siliconflowSettingsContainer)
            ...
            .addText(text => text
                ...
                .setValue(this.plugin.settings.ai.siliconflow?.apiAddress || '')
                .onChange(async (value) => {
                    if (!this.plugin.settings.ai.siliconflow) {
                        this.plugin.settings.ai.siliconflow = {};
                    }
                    this.plugin.settings.ai.siliconflow.apiAddress = value;
                    await this.plugin.saveSettings();
                }));
    }
